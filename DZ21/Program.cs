...
double myDistance = GetDistance(x1Number, y1Number, x2Number, y2Number, z1Number, z2Number);

Console.WriteLine($"Расстояние между точками ({x1Number}, {y1Number}) и ({x2Number}, {y2Number}) и ({z1Number}, {z2Number}) составляет {Math.Round(myDistance, 2)}.");
